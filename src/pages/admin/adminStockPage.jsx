...
//admin view of the same live inventory board, handy while running the
//concurrency test script against the backend
export default function AdminStockPage() {
	...
	useEffect(() => {
		...
		const interval = setInterval(loadStock, 2000);
		return () => clearInterval(interval);
	}, [autoRefresh]);

	const totalReserved = stockLevels.reduce((sum, row) => sum + row.reservedStock, 0);
	...
	return (
		<div className="w-full flex flex-col items-center p-10 bg-gradient-to-b from-primary to-white text-secondary">
			<div className="w-full max-w-7xl flex justify-between items-center mb-4">
				<div>
					<h1 className="text-2xl font-bold">Stock Monitor</h1>
					<p className="text-sm text-secondary/60">
						{totalReserved} unit(s) currently locked by live checkouts &middot; updated{" "}
